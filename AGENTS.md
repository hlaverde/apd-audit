...
Read **`docs/PROJECT_STATUS.md`** before anything else. It has the
current state (what's generated, what's still open, known issues, git
state) verified against the live repo, not remembered from a prior
chat. This file (below) has the durable house rules; that one has the
